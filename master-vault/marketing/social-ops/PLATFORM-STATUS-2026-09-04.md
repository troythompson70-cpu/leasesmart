...
**Date:** 2026-09-04  
**Owner gate:** Tee Gates approval required before any publish / paid / SMS  
...
**Identity standard:** `master-vault/IDENTITY-STANDARD.md`  
...
- Do **not** publish social content without Tee Gates approval.
- Use existing approved Labor Day campaign assets.
- Do **not** regenerate Tee Gates face / AI likeness without approval (OWNER APPROVAL REQUIRED).
...
|---|---|---|---|---|
| Link hub | `linktr.ee/tgttechnologies` | Not found (404) | Need owned Linktree / link hub | Tee Gates: choose handle + approve publish |
| Link hub | `linktr.ee/tgt` | Exists but **unrelated** | Japanese creator account — do **not** use | Avoid |
| TikTok | `@tgttechnologies` | Not found | Brand handle available to claim (verify in-app) | Tee Gates: create/claim + approve |
| TikTok | `@teegates` | Exists (Tee Gates) | Low activity / empty content as probed | Tee Gates: confirm ownership + bio |
| Facebook | `facebook.com/TGTTechnologies` | Exists | Page titled TGT Technologies (~91 followers observed) | Tee Gates: confirm admin access + bio |
| Instagram | `@tgttechnologies` | Exists (login-walled) | Public content not fully visible without auth | Tee Gates: confirm ownership + bio |
| Instagram | `@teegates` | Exists (login-walled) | Confirm if founder personal brand | Tee Gates: confirm |
| YouTube | `@teegates` | Exists (Trevor Gates) | Channel present; content sparse / empty as probed — **ambiguous platform display name; review required** | Tee Gates: confirm naming + upload rights |
| YouTube | `@TGTTechnologies` | Not found (404) | Do not link until claimed | Fixed homepage WATCH MORE → `@teegates` |
| LinkedIn | company `tgttechnologies` / `tgt-technologies` | Not found (404) | Company page likely needs creation | Tee Gates: create/claim when ready |

## Recommended link hub destinations (draft — not published)

Use only after Tee Gates approval. Destination order for consumer conversion:
...
| Tee Gates face / likeness | Use existing approved stills only | Do not regenerate likeness (OWNER APPROVAL REQUIRED) |
...
| Social posts | Platform native insights only after Tee Gates publish approval | Not activated |
...
1. **Tee Gates:** Provide Microsoft 365 access / share link for `TGT_WEBSITE_SOCIAL_LAUNCH_HANDOFF_2026-09-04.md` so Cursor can sync results to the permanent record.
2. **Tee Gates:** Confirm ownership of Facebook, Instagram `@tgttechnologies`, TikTok `@teegates`, YouTube `@teegates`.
3. **Tee Gates:** Approve Linktree/link-hub handle (not `linktr.ee/tgt`).
4. **Tee Gates:** Approve draft bios + Labor Day creative package before any publish.
5. **Cursor (after approvals):** Populate link hub destinations pointing at conversion homepage anchors.
6. **Cursor / Tee Gates:** Claim missing brand handles (TikTok brand, YouTube brand, LinkedIn company) only with Tee Gates present for 2FA/email (legal identity may be required by platforms for KYC/verification — use Troy Thompson only if the platform specifically requires legal identity).
